...
//             prog <- AwsApp.listBuckets().provideLayer(topEnv)
//           } yield prog

//           assertM(res.foldM(ex => ZIO.fail(ex.getMessage), _ => ZIO.succeed("ok")))(equalTo("ok"))
//         }
//       )
//     ).provideCustomLayerShared(S3Client.clientLayer)
// }

// object S3Client {
//   val client =
//     AwsAgent.createClient(region, endpointOverride).mapError(_ => new IOException("S3 client creation failed"))

//   val clientLayer = ZLayer.succeed(client)
// }
